...
This module provides a simple interface for creating and training deep neural
networks with PyTorch without reimplementing common functions.
...
class EarlyStopping:
    """
    Implements early stopping to terminate training when the validation loss
...
class Experiment:
    ...
    Requires a model, loss function, and optimizer.
    ...
    plt_args_training : dict, optional
        Additional arguments for training plot.
        Default is an empty dictionary.
    plt_args_validation : dict, optional
        Additional arguments for validation plot.
        Default is an empty dictionary.
    """
    ...
    def __post_init__(self):
        """
        Initializes the Experiment class by setting up the model,
        loss function, optimizer, and early stopping parameters.
        """
        ...
        if self.use_early_stopping:
            ...
            early_stopping_folder = os.path.join(
                self.checkpoints_folder, "early_stoppings", self.name
            )
            os.makedirs(early_stopping_folder, exist_ok=True)

            self.early_stopping = EarlyStopping(
                save_path=os.path.join(
                    early_stopping_folder,
        ...
        if "accuracy" in self.metrics:
            ...
            self.accuracy_train = MulticlassAccuracy(
                num_classes=self.n_classes
                ).to(
                self.device
            )
            self.accuracy_val = MulticlassAccuracy(
        ...
        if "precision" in self.metrics:
            ...
            self.precision_train = MulticlassPrecision(
                num_classes=self.n_classes).to(
                self.device
            )
        ...
        if "recall" in self.metrics:
            ...
            self.recall_train = MulticlassRecall(
                num_classes=self.n_classes).to(
                self.device
            )
            self.recall_val = MulticlassRecall(
                num_classes=self.n_classes).to(
                self.device
            )
        ...
        if "f1" in self.metrics:
            ...
            self.f1_train = MulticlassF1Score(num_classes=self.n_classes).to(
                self.device
            )
            self.f1_val = MulticlassF1Score(num_classes=self.n_classes).to(
                self.device
            )
        ...
        # Checkpointing setup
        self.checkpoints_folder = os.path.join(
            self.checkpoints_folder, "checkpoints", self.name
        )
        os.makedirs(self.checkpoints_folder, exist_ok=True)
        self.checkpoint_save_path = os.path.join(
            self.checkpoints_folder, self.checkpoint_name
        )
...
class Helper:
    ...
    def plot_images(
        ...
        """
        Displays a batch of images from the dataset, also showing the
        predicted label if provided.
        Args:
            ...
            validation: If True, also shows the predicted label
            (default False).
            y_pred: List of predicted labels
            (optional, required if validation=True).
        """

        fig, axes = plt.subplots(num_row, num_col, figsize=(
            10 * num_row, 2 * num_col
        ))
    ...
    def plot_loss(exp: Experiment):
        ...
        if not exp.train_loss_values or not exp.val_loss_values:
            raise ValueError(
                "Loss values are not available. Please train the model first."
            )

        # Check if the experiment has epoch count
        if not exp.epoch_count:
            raise ValueError(
                "Epoch count is not available. Please train the model first."
            )
    ...
    def evaluate_experiment(
        exp, dataset, classes, iterations, show_images=False
    ):
        """
        Evaluate the trained models on the test set and print classification
        ...
        # Check values
        if not np.all(np.isin(y_true, range(exp.n_classes))):
            raise ValueError(
                (
                    "y_true contains values outside the range of classes. "
                    f"Got {np.unique(y_true)}."
                )
            )
        ...
        # Print classification report, confusion matrix, and loss plots
        print(
        ...
        Helper.plot_confusion_matrix(
            y_true, y_pred, classes, title=f"Confusion Matrix for {exp.name}"
        )

        # debug only
        min_len = min(
            len(exp.epoch_count),
            len(exp.train_loss_values),
            len(exp.val_loss_values)
        )
        exp.epoch_count = exp.epoch_count[:min_len]
...
class Trainer:
    ...
    def fit(exp: Experiment, train_dl, val_dl, verbose=True):
        ...
        if exp.lr_scheduler:
            lr_scheduler = torch.optim.lr_scheduler.StepLR(
                exp.optimizer, step_size=exp.lr_step, gamma=exp.lr_gamma
            )

        if verbose:
            print(
                (
                    f"Training {exp.name}. Epochs: {exp.epochs} | "
                    f"Learning Rate: {exp.lr} | "
                    f"Batch Size: {train_dl.batch_size} | "
                    f"Loss Function: {exp.loss_fn.__class__.__name__} | "
                    f"Optimizer: {exp.optimizer.__class__.__name__}"
                )
            )
            ...
            if verbose:
                print(
                    (
                        ...
                        f"Val Accuracy: {epoch_val_accuracy} | "
            ...
            if exp.use_early_stopping:
                ...
                if exp.early_stopping.early_stop:
                    ...
                    exp.model.load_state_dict(
                        torch.load(exp.checkpoint_save_path)
                    )
                    break
    ...
    def evaluate(exp: Experiment, testloader):
        ...
        Args:
            exp: Experiment object containing model and evaluation parameters.
        ...
        # Store metrics values
        epoch_accuracy, epoch_precision, epoch_f1, epoch_recall = (
            Trainer._compute_classification_metrics(exp)
        )
...
class CrossValidation:
    ...
    def run(self):
        ...
        for exp in self.experiments:
            ...
            print(f"Cross Validation: {exp.name} with {self.n_splits} folds")
            ...
            for fold, (train_idx, val_idx) in enumerate(
                kf.split(self.train_ds)
            ):

                # Reset the model to initial state
                for i, layer in enumerate(exp.model.children()):
                    if (
                        hasattr(layer, "reset_parameters") and i >= 173
                    ):  # Skip the ResNet backbone
                        layer.reset_parameters()

                exp.optimizer = exp.optimizer.__class__(
                    exp.model.parameters(), lr=exp.lr
                )
                ...
                train_dl_split = data_utils.DataLoader(
                    self.train_ds, batch_size=self.batch_size,
                    sampler=train_sampler
                )
                val_dl_split = data_utils.DataLoader(
                    self.train_ds, batch_size=self.batch_size,
                    sampler=val_sampler
                )
                ...
                val_loss, accuracy, precision, f1, recall = Trainer.evaluate(
                    exp, val_dl_split
                )
                ...
                if accuracy is not Trainer.NA_MESSAGE:
                    cross_val_accuracy.append(
                        float(accuracy.cpu())  # type: ignore
                        if torch.is_tensor(accuracy)
                        else float(accuracy)
                    )

                if precision is not Trainer.NA_MESSAGE:
                    cross_val_precision.append(
                        float(precision.cpu())  # type: ignore
                ...
                if f1 is not Trainer.NA_MESSAGE:
                    cross_val_f1.append(
                        float(f1.cpu()) if torch.is_tensor(f1)  # type: ignore
                        else float(f1)
                    )

                if recall is not Trainer.NA_MESSAGE:
                    cross_val_recall.append(
                        float(recall.cpu())  # type: ignore
            ...
            if self.verbose:
                ...
                print(
                    (
                        f"Val Accuracy: "
                        f"{experiment_results[exp.name]['val_accuracy']}"
                    )
                )
                print(
                    "Val Precision: "
                    f"{experiment_results[exp.name]['val_precision']}"
                )
                print(f"Val F1: {experiment_results[exp.name]['val_f1']}")
                print(
                    f"Val Recall: {experiment_results[exp.name]['val_recall']}"
                )
